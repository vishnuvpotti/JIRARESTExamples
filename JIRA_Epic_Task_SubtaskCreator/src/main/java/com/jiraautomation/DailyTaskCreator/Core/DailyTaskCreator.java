package com.jiraautomation.DailyTaskCreator.Core;
...
import com.jiraautomation.DailyTaskCreator.DataClass.DailtTaskList;
import com.jiraautomation.DailyTaskCreator.DataClass.Epic;
import com.jiraautomation.DailyTaskCreator.DataClass.Fields;
import com.jiraautomation.DailyTaskCreator.DataClass.Parameter;
import com.jiraautomation.DailyTaskCreator.DataClass.Subtask;
import com.jiraautomation.DailyTaskCreator.DataClass.Task;
import com.jiraautomation.DailyTaskCreator.Utilities.JIRAHelper;
import com.jiraautomation.DailyTaskCreator.Utilities.Logger;
...
public class DailyTaskCreator {
			...
			for (Parameter parameterInfo : parameterList) {
				...
				try {
					...
					case "businessteam":
						...
						paramField = jiraHelperObject.CreateFieldValue(Fields.businessTeam, parameterInfo.getValue(), "complex", true);
						issuebuilder.setFieldInput(paramField);
						break;
					case "dailytaskssla":
						// Logger.WriteLog("Creating dailytaskssla field");
						paramField = jiraHelperObject.CreateFieldValue(Fields.businessTeam, FormatInputValue(parameterInfo.getValue()), "complex", true);
						issuebuilder.setFieldInput(paramField);
						break;
					case "assignee":
	...
	/**
	 * Function to format input value from xml. eg %date:yy-MM-dd - Test Summary
	 * converts to currentdate -Test Summary
	 * 
	 * @param inputString
	 *            : The string which needs to be formatted. IF plain string is
	 *            passed, it is returned as it is.
...
//				System.out.println("Adding comments : " + commentsToAdd + "  to key : " + key);
				...
				String outputMessage = commentsVerification == true ? "Comments are added successfully for key :"+key : "Comments not added due to some issues for key :"+key;
